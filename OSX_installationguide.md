...
Other potentially useful link - the Jenkins tool keeps track of the output from the underlying EIC build every night. One can take a look at the [console output](https://web.sdcc.bnl.gov/jenkins-sphenix/job/EIC/job/singularity-download-validation/) from running the Singularity container from scratch on the RCAS computers, which may provide some helpful feedback to compare to what kind of output you are getting locally.
